...
Training Data: A ChEMBL dataset containing 28 HIV-protease inhibitors with IC50 values was downloaded and preprocessed. The data was filtered to include only IC50 values in nanomolar units with a standard relation of =. pIC50 values were then calculated. ChEMBL ID of protein used: CHEMBL4296312

   Control ligands : 14 experimentally validated ligands with known Ki_nM values were used as a control set to compare predicted vs. experimental potency.

2. Feature Engineering
For ligand sets (training and control), RDKit molecular descriptors were calculated. These include:
...
3. Model Training
A Ridge Regression model was trained using the preprocessed ChEMBL dataset. The target variable for prediction was pIC50, and the RDKit descriptors served as features. A StandardScaler was applied to the features, and RidgeCV was used to find the optimal alpha.

4. Prediction & Evaluation
-  The trained model was used to predict pIC50 values for the 14
ligands.

-  The predicted pIC50 values were compared against their experimentally derived pIC50 values, which was calculated from Ki_nM
...
-   R-squared (R²): 0.16
       This indicates that our model explains only 16% of the variability in the experimental pIC50 values for the control ligands. This is a relatively low value, suggesting that while the model captures some aspects of potency, a large portion (84%) of the experimental variation is not explained by the current features or model. The scatter plot showed a general trend.
...
-   Additional Features: Incorporate more advanced molecular descriptors such as ECFP4 fingerprints, 3D descriptors etc.
-   Advanced Models: Experiment with other machine learning algorithms.
